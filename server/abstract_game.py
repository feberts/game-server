...
class AbstractGame:
    ...
    In some cases the framework performes checks before it calls a method. In such a case you can assume, that the argument passed is valid. Refer to the method descriptions to see which parameters this may apply to.
    """

    def __init(self, players)__:
        """
        Constructor.

        The framework assignes ids in the range 0..n to all players that join a game. It then passes the total number of players to the constructor.
    ...
    def current_player(self):
        ...
        This class must keep track of which player must perform the next move. This function reports this players id to the framework. The framework makes sure, that no other player can submit a move.
    ...
    def move(self, move):
        ...
        A players move is passed as a dictionary. The content of this dictionary entirely depends on the needs of the game. The API function to submit a move on the client side accepts the data as keyword arguments. Those keyword arguments are then converted to a dictionary.
        ...
        Returns:
        tuple(bool, str):
            bool: to inform the client wether the move was valid or not
