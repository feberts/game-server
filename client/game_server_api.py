...
class GameServerAPI:
    ...
    def start_game(self, server, port, game, token, players, name=''):
        ...
        function. They will be able to retrieve the same game state from the
        server as you.
    ...
    def join_game(self, server, port, game, token, name=''):
        ...
        must be provided. The token is used to identify a specific game session.
    ...
    def _process_args(self, server, port, game, token, name, players=1):
        ...
        assert type(port) == int and 0 <= port <= 65535, msg('port')
